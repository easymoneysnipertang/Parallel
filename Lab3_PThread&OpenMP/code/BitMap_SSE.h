...
alignas(16) BitMapSSE bitmapQListsSSE[];//开空间
...
void bitMapSSEProcessing(vector<InvertedIndex>& invertedLists, int count)
{
	...
	for (int i = 0; i < count; i++)
		bitmapQListsSSE[i] = bitmapListSSE[i];//建立id对应bitmap
}
InvertedIndex BITMAP_SSE(int* list, vector<InvertedIndex>& idx, int num)
{
	...
	for (int i = 1; i < num; i++)
	{
		for (int j = 0; j < bitmapQListsSSE[0].secondIndex.size(); j+=4)
		{
			...
			__m128i cSecIndex = _mm_loadu_epi32(&bitmapQListsSSE[list[0]].secondIndex[j]);//二级索引比较
			...
			for(int k=0;k<4;k++)
				if(resSec[k]!=0)
					for (int t = (j+k) * 32; t < (j+k) * 32 ; t+=4)
					{
						...
						__m128i cFirIndex = _mm_loadu_epi32(&bitmapQListsSSE[list[0]].firstIndex[t]);//二级索引比较;//一级索引比较
						...
						for(int c=0;c<4;c++)
							if (resFir[c]!=0)
							{
								...
								{
									__m128i cIndex = _mm_loadu_epi32(&bitmapQListsSSE[list[0]].firstIndex[l]);//关键字比较
...
//		{
//			bitmapQListsSSE[list[0]].secondIndex[j] &= bitmapQListsSSE[i].secondIndex[j];//按位与
//			if (bitmapQListsSSE[list[0]].secondIndex[j])
//			{
//				for (int t = j * 32; t < j * 32 + 32; t++)
//				{
//					bitmapQListsSSE[list[0]].firstIndex[t] &= bitmapQListsSSE[i].firstIndex[t];//按位与
//					if (bitmapQListsSSE[list[0]].firstIndex[t])
//					{
//						for (int l = t * 32; l < t * 32 + 32; l+=4)
//						{
//							__m128i cIndex = _mm_load_si128((__m128i*) & bitmapQListsSSE[list[0]].bits[l]);//按位与
